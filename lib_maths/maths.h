...
namespace sf {
	...
	template <typename T> double length(const Vector2<T> &v) {
		return sqrt(v.x * v.x + v.y * v.y); // What the fuck is "sqrt"???... Square root?
	}
	...
	template <typename T> Vector2<T> rotate(const Vector2<T> &v, const double degrees) {
		...
		return{ (T)(v.x * cs - v.y * sn), (T)(v.x * sn + v.y * cs) }; // may need to make the first '* c' to '*c'!!!!!!!!!!!
	}
	// Allow sf::vectors to be counted
